...
# Deep Hash Embedding (DHE) PyTorch implementation.
...
class DLRM_Net(nn.Module):
    ...
    # Create DHE stacks
    def create_dhe_stacks(self, ln, args_dhe):
        dhe_stacks = DHEBag(ln, args_dhe)
        return dhe_stacks
    ...
    def __init__(
        ...
        loss_function="bce",
        embedding_representation="table",
        args_dhe={}
    ):
        ...
        if (
            ...
            # embedding representation arguments
            self.embedding_representation = embedding_representation
            self.args_dhe = args_dhe

            # If running distributed, get local slice of embedding tables
            if ext_dist.my_size > 1:
                assert self.embedding_representation == "table", (
                    "Non-table representations unsupported for distributed execution.")
                n_emb = len(ln_emb)
            ...
            if ndevices <= 1:
                # handle embeddings
                # Option 1 (Default): Use Embedding Table
                if self.embedding_representation == 'table':
                    ...
                    if self.weighted_pooling == "learned":
                        ...
                        self.v_W_l = w_list
                # Option 2: Use DHE
                elif self.embedding_representation == 'dhe':
                    self.dhe_l = self.create_dhe_stacks(ln_emb, args_dhe)
                # Option 3: Use Hybrid
                elif self.embedding_representation == 'hybrid':
                    self.emb_l, w_list = self.create_emb(m_spa, ln_emb, weighted_pooling)
                    if self.weighted_pooling == "learned":
                        self.v_W_l = nn.ParameterList()
                        for w in w_list:
                    ...
                    self.dhe_l = self.create_dhe_stacks(ln_emb, args_dhe)
                # Option 4: Use Select
                elif self.embedding_representation == 'select':
                    self.emb_l, w_list = self.create_emb(m_spa, args_dhe['ln_tbl'], weighted_pooling)
                    if self.weighted_pooling == "learned":
                        self.v_W_l = nn.ParameterList()
                        for w in w_list:
                            self.v_W_l.append(Parameter(w))
                        ...
                        self.v_W_l = w_list
                    self.dhe_l = self.create_dhe_stacks(args_dhe['ln_dhe'], args_dhe)
                else:
                    sys.exit("ERROR: --embedding-representation={} is not supported".format(
    ...
    # Apply DHE stacks on sparse feature inputs
    def apply_dhe_stacks(self, lS_o, lS_i, dhe_l):
        return dhe_l(lS_o, lS_i)
    ...
    def forward(self, dense_x, lS_o, lS_i):
        if ext_dist.my_size > 1:
            ...
            assert self.embedding_representation == "table", (
                "Non-table representations unsupported for multi-node multi-device execution.")
            ...
            assert self.embedding_representation == "table", (
                "Non-table representations unsupported for single-node multi-device execution.")
    ...
    def sequential_forward(self, dense_x, lS_o, lS_i):
        ...
        # Option 1: Use Embedding Table
        if self.embedding_representation == 'table':
            ly = self.apply_emb(lS_o, lS_i, self.emb_l, self.v_W_l)
        # Option 2: Use DHE
        elif self.embedding_representation == 'dhe':
            ly = self.apply_dhe_stacks(lS_o, lS_i, self.dhe_l)
        # Option 3: Use Hybrid
        elif self.embedding_representation == 'hybrid':
            ly_tbl = self.apply_emb(lS_o, lS_i, self.emb_l, self.v_W_l)
            ly_dhe = self.apply_dhe_stacks(lS_o, lS_i, self.dhe_l)
            ly = ly_tbl + ly_dhe # concatenate results from both embedding processing techniques
        # Option 4: Use Select
        elif self.embedding_representation == 'select':
            # preprocess offsets and indices for exclusive embedding processing techniques
            lS_o_tbl = [lS_o[i] for i in self.args_dhe['sparse_fea_tbl']]
            lS_o_dhe = [lS_o[i] for i in self.args_dhe['sparse_fea_dhe']]
            lS_i_tbl = [lS_i[i] for i in self.args_dhe['sparse_fea_tbl']]
            lS_i_dhe = [lS_i[i] for i in self.args_dhe['sparse_fea_dhe']]
            ly_tbl = self.apply_emb(lS_o_tbl, lS_i_tbl, self.emb_l, self.v_W_l)
            ly_dhe = self.apply_dhe_stacks(lS_o_dhe, lS_i_dhe, self.dhe_l)
            ly = ly_tbl + ly_dhe
        else:
            sys.exit("ERROR: --embedding-representation={} is not supported".format(
                self.embedding_representation))
...
def run():
    ...
    parser.add_argument(
        "--arch-embedding-size", type=dash_separated_ints, default="3-4-10-15-18-24-27-105-305-583-633-1460-2173\
        -3194-5652-5683-12517-14992-93145-142572-286181-2202608-5461306-7046547-8351593-10131227"
    )
    ...
    parser.add_argument("--embedding-representation", type=str, default="table", help='Sparse Feature Embedding Representation. \
        Current options include table, dhe, hybrid, and select')
    # DHE/Hybrid/Select Arguments
    ...
    parser.add_argument('--dhe-mlp-dims', type=str, default='1024-128-16', help='DHE Decoder MLP Dimensions')
    parser.add_argument('--dhe-precision', type=str, default='np.float32', help='DHE Decoder MLP Precision')
    parser.add_argument('--dhe-transform', type=str, default='uniform', help='DHE Encoder Transform Distribution')
    parser.add_argument("--select-sparse-feature-mode", type=str, default="dhe", help='Sparse Feature Representation of Select Subgroup')
    parser.add_argument("--select-sparse-feature-ids", type=dash_separated_ints, default="21-22-23-24-25", help='Sparse Feature IDs of Select Subgroup')
    ...
    # build args_dhe dict
    args_dhe = {}
    if args.embedding_representation in ['dhe', 'hybrid', 'select']:
        print('-------------------- Enabling {} --------------------'.format(args.embedding_representation))
        # DHE Parameters
        ...
        args_dhe['mlp_dims']    = np.fromstring(args.dhe_mlp_dims, dtype=int, sep="-")
        args_dhe['num_lookups'] = args.num_indices_per_lookup
        args_dhe['precision']   = eval(args.dhe_precision)
        args_dhe['seed']        = args.numpy_rand_seed
        args_dhe['transform']   = args.dhe_transform

        dhe_m_spa = args_dhe['mlp_dims'][-1] # "embedding dimension" of DHE stack -- i.e., last dim of decoder MLP
        
        # Special preprocessing for select representation.
        if args.embedding_representation == 'select':
            selected_features      = np.sort(np.fromstring(args.select_sparse_feature_ids, dtype=int, sep = '-'))
            complementary_features = np.setdiff1d(np.arange(ln_emb.size), selected_features)
            # specified sparse features are for DHE
            if args.select_sparse_feature_mode == 'dhe':
                sparse_fea_tbl = complementary_features
                sparse_fea_dhe = selected_features
            # specified sparse features are for table
            elif args.select_sparse_feature_mode == 'table':
                sparse_fea_tbl = selected_features
                sparse_fea_dhe = complementary_features
            else:
                sys.exit("ERROR: --select-sparse-feature-mode={} is not supported".format(
                    args.select_sparse_feature_mode))
            num_fea_tbl = sparse_fea_tbl.size # number of sparse features using table
            num_fea_dhe = sparse_fea_dhe.size # number of sparse features using DHE
            args_dhe['sparse_fea_tbl'] = sparse_fea_tbl
            args_dhe['sparse_fea_dhe'] = sparse_fea_dhe
            args_dhe['ln_tbl'] = np.array([ln_emb[i] for i in sparse_fea_tbl])
            args_dhe['ln_dhe'] = np.array([ln_emb[i] for i in sparse_fea_dhe])
            assert num_fea_tbl + num_fea_dhe == ln_emb.size, (
                "Incorrect breakdown of table and DHE sparse feature processing stacks for select")

        # Summary DHE stack messages
        print('---------- DHE Stack Parameters: ----------')
    ...
    if args.arch_interaction_op == "dot":
        if args.embedding_representation == 'hybrid':
            num_fea = 2*ln_emb.size + 1 # account for two resulting embeddings for each sparse feature
        # approach 1: all
        ...
        if args.embedding_representation == 'table':
            num_int = num_fea * m_den_out
        elif args.embedding_representation == 'dhe':
            num_int = (num_fea - 1) * dhe_m_spa + m_den_out
        elif args.embedding_representation == 'hybrid':
            num_int = num_fea * m_den_out + (num_fea - 1) * dhe_m_spa
        elif args.embedding_representation == 'select':
            num_int = (num_fea_tbl + 1) * m_den_out + num_fea_dhe * dhe_m_spa
        else:
            sys.exit("ERROR: --embedding-representation={} is not supported".format(
    ...
    if args.embedding_representation in ['dhe', 'hybrid', 'select']:
        if args_dhe['k'] != args_dhe['mlp_dims'][0]:
            sys.exit("ERROR: DHE Encoder k {} does not match first dim of DHE Decoder mlp {}".format(
                str(args_dhe['k']), str(args_dhe['mlp_dims'][0])))
        if args.arch_interaction_op == 'dot':
            if dhe_m_spa != m_den_out:
                sys.exit("ERROR: For dot interaction, DHE embedding dimension {} has to match last dim of bottom mlp {}".format(
                dhe_m_spa, m_den_out))
    ...
    if ndevices > 1 and args.embedding_representation in ['dhe', 'hybrid', 'select']:
        print('Using 1 instead of {} GPUs for enabling {}'.format(ndevices, args.embedding_representation))
        ndevices = 1
