...
def _mp_fn_inference(index, args):
    ...
    if xm.get_local_ordinal() in list_of_ordinals:
        ...
        model.to(device)
        print('Process {} successfully created DLRM on {}'.format(index, device_name))

        xm.rendezvous('DLRM Creation Complete', replicas=list_of_ordinals)
        if xm.is_master_ordinal():
            print('========================================')
...
def main():
    ...
    if args.inference == True:
        xmp.spawn(_mp_fn_inference, args=(args,), nprocs=8, start_method='spawn')
