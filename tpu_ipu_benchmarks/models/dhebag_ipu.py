...
class DHEBag_IPU(nn.Module):
    def __init__(
        self, 
        table_sizes: List, 
        args_dhe: TypedDict,
    ):
        super(DHEBag_IPU, self).__init__()
    ...
    def generate_hash_params(
        ...
        for _ in range(num_tables):
            ...
            hash_params_table_i = hash_params_table_i.to(torch.int32)
    ...
    def hash(
        ...
        if args_dhe["hash_fn"] == "universal":
            ...
            # IPU cannot use 'torch.outer'
            x = x.reshape(len(x), 1).to(torch.int32)
            x_hashed = torch.mul(x, a).add(b).fmod(p).fmod(m)
        else:
            sys.exit("DHE: Please use valid hash function!")
    ...
    def forward(
        ...
        for k, sparse_index_group_batch in enumerate(lS_i):
            sparse_offset_group_batch = lS_o # lS_o[k]  # for IPU, use constant offsets
