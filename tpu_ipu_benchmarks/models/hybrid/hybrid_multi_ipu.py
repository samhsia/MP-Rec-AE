...
class Hybrid_MULTI_IPU(nn.Module):
    ...
    def __init__(
        ...
        super(Hybrid_MULTI_IPU, self).__init__()
       ...
       # Make Embedding Tables
        self.embedding_tables_1, self.tables_size_1 = self.create_embedding_tables(self.emb_dim, table_sizes_1, self.precision)
        self.tables_flops_1 = len(self.table_sizes_1) * self.emb_dim * (self.num_lookups - 1)

        self.embedding_tables_2, self.tables_size_2 = self.create_embedding_tables(self.emb_dim, table_sizes_2, self.precision)
        self.tables_flops_2 = len(self.table_sizes_2) * self.emb_dim * (self.num_lookups - 1)

        self.embedding_tables_3, self.tables_size_3 = self.create_embedding_tables(self.emb_dim, table_sizes_3, self.precision)
        self.tables_flops_3 = len(self.table_sizes_3) * self.emb_dim * (self.num_lookups - 1)

        self.embedding_tables_4, self.tables_size_4 = self.create_embedding_tables(self.emb_dim, table_sizes_4, self.precision)
    ...
    def forward(self, x_dense, x_indices, labels=None):
        ...
        with poptorch.Block(ipu_id=3):
            ...
            embeddings_dhe = self.apply_dhe_stacks(self.constant_offsets, x_indices)
            mlp_bot_out = self.apply_mlp(x_dense, self.mlp_bot)
            fea_int_out = torch.cat([mlp_bot_out] + embeddings_1 + embeddings_2 + embeddings_3 + embeddings_4 + embeddings_dhe, dim=1)
